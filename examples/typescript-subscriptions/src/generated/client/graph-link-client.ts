...
class GraphLinkSubscriptions extends _ResolverBase {
   ...
   personCreated(onEvent: (data: PersonCreatedResponse) => void, onError?: (error: unknown) => void): () => void {
      const __gl_variables__: Record<string, unknown> = {};
      const __gl_fragsValues__ = [
        '_all_fields_Person',
      ].map(name => this.__gl_fragMap__[name]!).join('');
      const __gl_query__ = `subscription personCreated{personCreated{..._all_fields_Person}} ${__gl_fragsValues__}`;
      const __gl_payload__: GraphLinkPayload = {
        query: __gl_query__,
        operationName: 'personCreated',
        variables: __gl_variables__,
      };
      const __gl_gen__ = this.__gl_handler__.handle(__gl_payload__);
      (async () => {
        try {
          for await (const __gl_data__ of __gl_gen__) {
            onEvent(__gl_data__ as unknown as PersonCreatedResponse);
          }
        } catch (e) {
          onError?.(e);
        }
      })();
      return () => { void __gl_gen__.return(undefined); };
   }
}
export class GraphLinkClient {
   ...
   constructor(adapter: GraphLinkAdapter, wsAdapter: GraphLinkWsAdapter, store?: GraphLinkCacheStore) {
      ...
      this.__gl_fragMap__['Inline_680ef8e1_9dfd_e398_ac50_df3459d74cbf'] = '... on Car  {..._all_fields_Car __typename} ';
      this.__gl_fragMap__['Inline_8fb1320e_123b_6288_a87a_05387a31fef6'] = '... on Moto  {..._all_fields_Moto __typename} ';
