...
import { GetTodosResponse } from '../types/get-todos-response.js';
import { GetTodoResponse } from '../types/get-todo-response.js';
import { CreateTodoResponse } from '../types/create-todo-response.js';
import { ToggleTodoResponse } from '../types/toggle-todo-response.js';
import { DeleteTodoResponse } from '../types/delete-todo-response.js';
import { OnTodoAddedResponse } from '../types/on-todo-added-response.js';
import { CreateTodoInput } from '../inputs/create-todo-input.js';
...
class GraphLinkQueries extends _ResolverBase {
   ...
   getTodos(): Observable<GetTodosResponse> {
      return new Observable<GetTodosResponse>(subscriber => {
        (async () => {
          const __gl_operationName__ = 'getTodos';
         ...
         subscriber.next(__gl_responseMap__ as unknown as GetTodosResponse);
         ...
         const __gl_result__ = this._parseAndCache(__gl_responseText__, __gl_responseMap__, __gl_remaining__) as unknown as GetTodosResponse;
   ...
   getTodo(args: { id: string }): Observable<GetTodoResponse> {
      return new Observable<GetTodoResponse>(subscriber => {
        (async () => {
          const __gl_operationName__ = 'getTodo';
          const __gl_variables__: Record<string, unknown> = {
        'id': args.id,
      };
          const __gl_partialQueries__ = [
            new _GraphLinkPartialQuery(
          'todo(id: $id){id title completed}',
          ...
          const __gl_remaining__ = __gl_partialQueries__.filter(pq => !(pq.elementKey in __gl_responseMap__));
          if (__gl_remaining__.length === 0) {
         subscriber.next(__gl_responseMap__ as unknown as GetTodoResponse);
         subscriber.complete(); return;
      }
          const __gl_payload__ = this._buildPayload(__gl_remaining__, __gl_operationName__, ''); 
          try {
         const __gl_responseText__ = await this.__gl_adapter__(JSON.stringify(__gl_payload__));
         const __gl_result__ = this._parseAndCache(__gl_responseText__, __gl_responseMap__, __gl_remaining__) as unknown as GetTodoResponse;
         subscriber.next(__gl_result__);
         subscriber.complete();
      } catch (e) {
         Object.assign(__gl_responseMap__, __gl_staleData__);
   ...
   todos(): Observable<GetTodosResponse> {
      return new Observable<GetTodosResponse>(subscriber => {
        (async () => {
          const __gl_operationName__ = 'todos';
          const __gl_variables__: Record<string, unknown> = {};
          const __gl_partialQueries__ = [
            new _GraphLinkPartialQuery(
          'todos{..._all_fields_Todo}',
          { },
          0,
          [],
          'todos__todos',
          'todos',
          new Set(['_all_fields_Todo']),
          [],
          false,
        ),
          ];
   ...
   todo(args: { id: string }): Observable<GetTodoResponse> {
      return new Observable<GetTodoResponse>(subscriber => {
        (async () => {
          const __gl_operationName__ = 'todo';
          const __gl_variables__: Record<string, unknown> = {
        'id': args.id,
      };
          const __gl_partialQueries__ = [
            new _GraphLinkPartialQuery(
          'todo(id: $id){..._all_fields_Todo}',
          { 'id': __gl_variables__['id'], },
          0,
          [],
          'todo__todo',
...
class GraphLinkMutations extends _ResolverBase {
   ...
   createTodo(args: { input: CreateTodoInput }): Observable<CreateTodoResponse> {
      return new Observable<CreateTodoResponse>(subscriber => {
        (async () => {
          const __gl_operationName__ = 'createTodo';
          const __gl_variables__: Record<string, unknown> = {
        'input': args.input,
      };
          const __gl_query__ = 'mutation createTodo($input: CreateTodoInput!){createTodo(input: $input){id title completed}}';
          ...
          if (__gl_result__['errors']) { subscriber.error(__gl_result__['errors']); return; }
          subscriber.next(__gl_result__['data'] as CreateTodoResponse);
          subscriber.complete();
        })();
      });
   }
   toggleTodo(args: { id: string }): Observable<ToggleTodoResponse> {
      return new Observable<ToggleTodoResponse>(subscriber => {
        (async () => {
          const __gl_operationName__ = 'toggleTodo';
          const __gl_variables__: Record<string, unknown> = {
        'id': args.id,
      };
          const __gl_query__ = 'mutation toggleTodo($id: ID!){toggleTodo(id: $id){id title completed}}';
          const __gl_payload__ = JSON.stringify({ query: __gl_query__, operationName: __gl_operationName__, variables: __gl_variables__ });
          const __gl_response__ = await this.__gl_adapter__(__gl_payload__);
          const __gl_result__ = JSON.parse(__gl_response__);
          if (__gl_result__['errors']) { subscriber.error(__gl_result__['errors']); return; }
          subscriber.next(__gl_result__['data'] as ToggleTodoResponse);
          subscriber.complete();
        })();
      });
   }
   deleteTodo(args: { id: string }): Observable<DeleteTodoResponse> {
      return new Observable<DeleteTodoResponse>(subscriber => {
        (async () => {
          const __gl_operationName__ = 'deleteTodo';
          const __gl_variables__: Record<string, unknown> = {
        'id': args.id,
      };
          const __gl_query__ = 'mutation deleteTodo($id: ID!){deleteTodo(id: $id)}';
          const __gl_payload__ = JSON.stringify({ query: __gl_query__, operationName: __gl_operationName__, variables: __gl_variables__ });
          const __gl_response__ = await this.__gl_adapter__(__gl_payload__);
          const __gl_result__ = JSON.parse(__gl_response__);
          if (__gl_result__['errors']) { subscriber.error(__gl_result__['errors']); return; }
          subscriber.next(__gl_result__['data'] as DeleteTodoResponse);
          subscriber.complete();
        })();
      });
   }
...
class GraphLinkSubscriptions extends _ResolverBase {
   ...
   onTodoAdded(): Observable<OnTodoAddedResponse> {
      const __gl_variables__: Record<string, unknown> = {};
      const __gl_query__ = 'subscription onTodoAdded{todoAdded{id title completed}}';
      ...
      return new Observable<OnTodoAddedResponse>(subscriber => {
        (async () => {
          try {
            for await (const __gl_data__ of __gl_gen__) {
              subscriber.next(__gl_data__ as unknown as OnTodoAddedResponse);
            }
            subscriber.complete();
          } catch (e) {
            subscriber.error(e);
   ...
   todoAdded(): Observable<OnTodoAddedResponse> {
      const __gl_variables__: Record<string, unknown> = {};
      const __gl_fragsValues__ = [
        '_all_fields_Todo',
      ].map(name => this.__gl_fragMap__[name]!).join('');
      const __gl_query__ = `subscription todoAdded{todoAdded{..._all_fields_Todo}} ${__gl_fragsValues__}`;
      const __gl_payload__: GraphLinkPayload = {
        query: __gl_query__,
...
export class GraphLinkClient {
   ...
   constructor(adapter: GraphLinkAdapter, wsAdapter: GraphLinkWsAdapter, store?: GraphLinkCacheStore) {
      this.__gl_fragMap__['_all_fields_Todo'] = 'fragment _all_fields_Todo on Todo{id title completed}';
