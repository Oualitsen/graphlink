...
class GraphLinkQueries extends _ResolverBase {
   ...
   async getTodos(): Promise<GetTodosResponse> {
      ...
      if (__gl_remaining__.length === 0) {
         return(__gl_responseMap__ as unknown as GetTodosResponse);
      }
   ...
   async getTodo(args: { id: string }): Promise<GetTodoResponse> {
      ...
      const __gl_remaining__ = __gl_partialQueries__.filter(pq => !(pq.elementKey in __gl_responseMap__));
      if (__gl_remaining__.length === 0) {
         return(__gl_responseMap__ as unknown as GetTodoResponse);
      }
      const __gl_payload__ = this._buildPayload(__gl_remaining__, __gl_operationName__, ''); 
      try {
         const __gl_responseText__ = await this.__gl_adapter__(JSON.stringify(__gl_payload__));
...
class GraphLinkMutations extends _ResolverBase {
   ...
   async createTodo(args: { input: CreateTodoInput }): Promise<CreateTodoResponse> {
      ...
      return __gl_result__['data'] as CreateTodoResponse;
   }
   async toggleTodo(args: { id: string }): Promise<ToggleTodoResponse> {
      ...
      const __gl_result__ = JSON.parse(__gl_response__);
      if (__gl_result__['errors']) throw __gl_result__['errors'] as GraphLinkError[];
      return __gl_result__['data'] as ToggleTodoResponse;
   }
   async deleteTodo(args: { id: string }): Promise<DeleteTodoResponse> {
      ...
      const __gl_result__ = JSON.parse(__gl_response__);
      if (__gl_result__['errors']) throw __gl_result__['errors'] as GraphLinkError[];
      return __gl_result__['data'] as DeleteTodoResponse;
   }
}
class GraphLinkSubscriptions extends _ResolverBase {
   ...
   onTodoAdded(onEvent: (data: OnTodoAddedResponse) => void, onError?: (error: unknown) => void): () => void {
      const __gl_variables__: Record<string, unknown> = {};
      ...
      (async () => {
        try {
          for await (const __gl_data__ of __gl_gen__) {
            onEvent(__gl_data__ as unknown as OnTodoAddedResponse);
          }
        } catch (e) {
          onError?.(e);
        }
