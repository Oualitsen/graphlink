...
extension GLHoistArgs on GLParser {
  ...
  /// Removes auto-generated operations whose propagated-arg count exceeds
  /// [autoQueryArgumentLimit]. Also removes the synthesized `<Op>FieldArgs`
  /// input so no orphan type is emitted. Skipped operations are recorded in
  /// [skippedAutoQueries] for the CLI to surface as warnings.
  ///
  /// Must run after [groupHoistedArgsIntoInputs] (so the synthetic arg and its
  /// input exist) and before `createProjectedTypes()`.
  void pruneOverloadedAutoQueries() {
    final cap = autoQueryArgumentLimit;
    if (cap == null) return;

    final toRemove = <GLOperationKey>[];
    for (final entry in queries.entries) {
      final def = entry.value;
      if (!def.isAutoGenerated) continue;

      GLArgumentDefinition? syntheticArg;
      for (final a in def.arguments) {
        if (a.hoistArgsInput != null) {
          syntheticArg = a;
          break;
        }
      }
      final count = syntheticArg?.hoistArgsInput?.fields.length ?? 0;
      if (count <= cap) continue;

      toRemove.add(entry.key);
      if (syntheticArg?.hoistArgsInput != null) {
        inputs.remove(syntheticArg!.hoistArgsInput!.token);
      }
      skippedAutoQueries.add(SkippedAutoQuery(
        name: def.token,
        type: def.type,
        count: count,
        cap: cap,
