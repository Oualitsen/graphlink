...
class GLGrammar extends GrammarDefinition {
  ...
  final Map<String, GLDirectiveDefinition> directives = {
    includeDirective: GLDirectiveDefinition(
      ...
      [GLArgumentDefinition("if".toToken(), GQType("Boolean".toToken(), false), [])],
    ...
    glTypeNameDirective: GLDirectiveDefinition(
      ...
      [
        GLArgumentDefinition(
            glTypeNameDirectiveArgumentName.toToken(), GQType("String".toToken(), false), [])
      ],
      {
    ...
    glEqualsHashcode: GLDirectiveDefinition(
      ...
      [
        GLArgumentDefinition(
            glEqualsHashcodeArgumentName.toToken(), GQType("[String]".toToken(), false), [])
      ],
  ...
  List<GLSchemaMapping> getServiceMappingByType(String typeName) => _schemaMappings.values
  ...
  Parser<GLDirectiveValue> directiveValue() =>
      seq2(directiveValueName(), argumentValues().optional())
          .map2((name, args) =>
              GLDirectiveValue(name.ofNewName(name.token.trim()), [], args ?? [], generated: false))
          .map((directiveValue) {
  ...
  Parser<GLDirectiveDefinition> directiveDefinition() => seq4(
          ...
          .map4((name, args, repeatable, scopes) =>
              GLDirectiveDefinition(name, args ?? [], scopes, repeatable != null))
          .map((definition) {
  ...
  Parser<Set<GLDirectiveScope>> directiveScopes() => seq2(directiveScope(),
  ...
  Parser<GLComment> singleLineComment() => (char('#') & ref0(newlineLexicalToken).neg().star())
      .flatten()
  ...
  Parser<GLScalarDefinition> scalarDefinition() => (ref1(token, extendKw()).optional() &
              ref1(token, scalarKw()) &
        ...
        var scalar =
            GLScalarDefinition(token: scalarName, directives: array[3], extension: extension);
