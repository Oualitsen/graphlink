...
class UploadTest {
    ...
    class UploadOneFile {
        @Test void returns_true_when_file_provided() throws Exception {
            UploadOneFileResponse res = client.mutations.uploadOneFile("user-1", makeFile(1, 2, 3)).block();
            assertTrue(res.getUploadOneFile());
        }
        @Test void returns_true_for_different_userId() throws Exception {
            UploadOneFileResponse res = client.mutations.uploadOneFile("user-42", makeFile(10, 20, 30)).block();
            assertTrue(res.getUploadOneFile());
        }
        @Test void returns_true_for_larger_file() throws Exception {
            int[] ints = new int[1024];
            for (int i = 0; i < ints.length; i++) ints[i] = i % 256;
            UploadOneFileResponse res = client.mutations.uploadOneFile("user-1", makeFile(ints)).block();
            assertTrue(res.getUploadOneFile());
        }
    }
