...
class ListsTest {
    ...
    class NonNullableListOfObjects {
        ...
        void listUsers_isNotNull() {
            assertNotNull(client.queries.listUsers().block().getListUsers());
        }

        @Test
        void listUsers_hasTwoItems() {
            assertEquals(2, client.queries.listUsers().block().getListUsers().size());
        }

        @Test
        void listUsers_firstItem_isUser() {
            assertInstanceOf(User.class, client.queries.listUsers().block().getListUsers().get(0));
        }

        @Test
        void listUsers_firstItem_hasCorrectId() {
            assertEquals("user-1", client.queries.listUsers().block().getListUsers().get(0).getId());
        }

        @Test
        void listUsers_secondItem_hasCorrectId() {
            assertEquals("user-2", client.queries.listUsers().block().getListUsers().get(1).getId());
        }

        @Test
        void listUsers_items_haveDeserializedEnums() {
            List<User> users = client.queries.listUsers().block().getListUsers();
    ...
    class ListWithNullableItemFields {
        ...
        void getTags_hasTwoItems() {
            assertEquals(2, client.queries.getTags().block().getGetTags().size());
        }

        @Test
        void getTags_firstItem_isTag() {
            assertInstanceOf(Tag.class, client.queries.getTags().block().getGetTags().get(0));
        }

        @Test
        void getTags_firstItem_hasColor() {
            assertEquals("#0175C2",
                    client.queries.getTags().block().getGetTags().get(0).getColor());
        }

        @Test
        void getTags_secondItem_colorIsNull() {
            assertNull(client.queries.getTags().block().getGetTags().get(1).getColor());
        }

        @Test
        void getTags_secondItem_hasLabel() {
            assertEquals("graphql",
                    client.queries.getTags().block().getGetTags().get(1).getLabel());
        }
    }
    ...
    class ListOfScalars {
        ...
        void tags_onUser_isListOfStrings() {
            List<String> tags = client.queries.getUser("user-1").block().getGetUser().getTags();
            assertNotNull(tags);
        ...
        void tags_empty_returnsEmptyList() {
            List<String> tags = client.queries.getUser("user-2").block().getGetUser().getTags();
            assertNotNull(tags);
        ...
        void scores_onUser_isListOfIntegers() {
            List<Integer> scores = client.queries.getUser("user-1").block().getGetUser().getScores();
    ...
    class SearchUsers {
        ...
        void searchUsers_returnsMatchingUsers() {
            SearchUsersResponse res = client.queries.searchUsers("Alice", 10).block();
        ...
        void searchUsers_byName_returnsOnlyMatches() {
            SearchUsersResponse res = client.queries.searchUsers("Bob", 10).block();
            assertNotNull(res.getSearchUsers());
            assertEquals(1, res.getSearchUsers().size());
