...
public final class Fixtures {
    ...
    static final String SERVER_URL = "http://localhost:9999/graphql";
    static final String WS_URL = "ws://localhost:9999/graphql";
    static final String UPLOAD_URL = "http://localhost:9998/graphql";

    // ── Client factories ──────────────────────────────────────────────────────

    /** Create a client connected to the real Spring Boot server (queries/mutations). */
    public static GraphLinkClient newRealClient() {
    ...
    /** Create a client connected to the upload server on port 9998. */
    public static GraphLinkClient newUploadClient() {
        JacksonGraphLinkJsonCodec codec = new JacksonGraphLinkJsonCodec();
        return new GraphLinkClient(
                new DefaultGraphLinkClientAdapter(SERVER_URL),
                new RealMultipartAdapter(UPLOAD_URL),
                codec, codec,
                new DefaultGraphLinkWebSocketAdapter(WS_URL));
    }

    /** Create a client wired to mock adapters — used for cache/call-count tests. */
    public static GraphLinkClient newClient(GraphLinkClientAdapter adapter, MockWebSocketAdapter wsAdapter) {
        JacksonGraphLinkJsonCodec codec = new JacksonGraphLinkJsonCodec();
