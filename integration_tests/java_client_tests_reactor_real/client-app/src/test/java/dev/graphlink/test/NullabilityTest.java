...
class NullabilityTest {
    ...
    class NonNullableQueryReturn {
        ...
        void getUser_hasCorrectId() {
            assertEquals("user-1", client.queries.getUser("user-1").block().getGetUser().getId());
        }
    }
    ...
    class NullableQueryReturn_Null {
        ...
        void findUser_returnsNull_forUnknownId() {
            FindUserResponse res = client.queries.findUser("user-99").block();
    ...
    class NullableQueryReturn_Present {
        ...
        void findUser_returnsUser_forKnownId() {
            FindUserResponse res = client.queries.findUser("user-1").block();
    ...
    class NullableNestedObject_Null {
        ...
        void billingAddress_isNull_forAlice() {
            assertNull(client.queries.getUser("user-1").block().getGetUser().getBillingAddress());
        }
    }
    ...
    class NullableNestedObject_Present {
        ...
        void billingAddress_isNotNull_forBob() {
            assertNotNull(client.queries.getUser("user-2").block().getGetUser().getBillingAddress());
        }

        @Test
        void billingAddress_hasCorrectCity() {
            assertEquals("Capital City",
                    client.queries.getUser("user-2").block().getGetUser().getBillingAddress().getCity());
        }
    }
    ...
    class NullableScalarOnNestedType {
        ...
        void address_zip_isNull_forBob() {
            assertNull(client.queries.getUser("user-2").block().getGetUser().getAddress().getZip());
        }
    ...
    class NullableListField {
        ...
        void scores_isNull_forBob() {
            assertNull(client.queries.getUser("user-2").block().getGetUser().getScores());
        }

        @Test
        void scores_hasList_forAlice() {
            assertNotNull(client.queries.getUser("user-1").block().getGetUser().getScores());
            assertEquals(3, client.queries.getUser("user-1").block().getGetUser().getScores().size());
        }
    }
    ...
    class NullableCoAuthor {
        ...
        void coAuthor_isNull_forPost1() {
            assertNull(client.queries.getPost("post-1").block().getGetPost().getCoAuthor());
        }
