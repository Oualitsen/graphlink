...
class NestedTypesTest {
    ...
    class RequiredNestedObject {
        ...
        void address_isNotNull() {
            assertNotNull(client.queries.getUser("user-1").block().getGetUser().getAddress());
        }

        @Test
        void address_hasCorrectStreet() {
            assertEquals("123 Main St",
                    client.queries.getUser("user-1").block().getGetUser().getAddress().getStreet());
        }

        @Test
        void address_hasCorrectCity() {
            assertEquals("Springfield",
                    client.queries.getUser("user-1").block().getGetUser().getAddress().getCity());
        }

        @Test
        void address_hasCorrectCountry() {
            assertEquals("US",
                    client.queries.getUser("user-1").block().getGetUser().getAddress().getCountry());
        }

        @Test
        void address_hasCorrectZip() {
            assertEquals("12345",
                    client.queries.getUser("user-1").block().getGetUser().getAddress().getZip());
        }

        @Test
        void address_deserializesToAddressType() {
            assertInstanceOf(Address.class,
                    client.queries.getUser("user-1").block().getGetUser().getAddress());
        }
    }
    ...
    class MultiLevelNesting {
        ...
        @Test
        void post_isNotNull() {
            assertNotNull(client.queries.getPost("post-1").block().getGetPost());
        }

        @Test
        void post_deserializesToPostType() {
            assertInstanceOf(Post.class, client.queries.getPost("post-1").block().getGetPost());
        }

        @Test
        void post_hasCorrectTitle() {
            assertEquals("Hello World",
                    client.queries.getPost("post-1").block().getGetPost().getTitle());
        }

        @Test
        void post_author_isNotNull() {
            assertNotNull(client.queries.getPost("post-1").block().getGetPost().getAuthor());
        }

        @Test
        void post_author_deserializesToUserFragmentType() {
            assertInstanceOf(User.class,
                    client.queries.getPost("post-1").block().getGetPost().getAuthor());
        }

        @Test
        void post_author_hasCorrectId() {
            assertEquals("user-1",
                    client.queries.getPost("post-1").block().getGetPost().getAuthor().getId());
        }

        @Test
        void post_author_status_deserializesToEnum() {
            assertEquals(UserStatus.ACTIVE,
                    client.queries.getPost("post-1").block().getGetPost().getAuthor().getStatus());
        }

        @Test
        void post_author_address_city_isAccessible() {
            assertEquals("Springfield",
                    client.queries.getPost("post-1").block().getGetPost().getAuthor().getAddress().getCity());
        }

        @Test
        void post_viewCount_deserializesToInteger() {
            assertEquals(128, client.queries.getPost("post-1").block().getGetPost().getViewCount());
        }
    }
    ...
    class SkipOnClientField {
        ...
        void auditEntry_deserializesCorrectly() {
            GetAuditEntryResponse res = client.queries.getAuditEntry("audit-1").block();
        ...
        void auditEntry_withServerOnlyField_isIgnoredSilently() {
            GetAuditEntryResponse res = client.queries.getAuditEntry("audit-2").block();
            AuditEntry entry = res.getGetAuditEntry();
            assertNotNull(entry);
