...
class CustomQueriesTest {
    ...
    class FetchUserAndPost {
        ...
        void response_isNotNull() {
            assertNotNull(client.queries.fetchUserAndPost("user-1", "post-1").block());
        }

        @Test
        void user_isNotNull() {
            assertNotNull(client.queries.fetchUserAndPost("user-1", "post-1").block().getUser());
        }

        @Test
        void user_hasCorrectId() {
            assertEquals("user-1",
                    client.queries.fetchUserAndPost("user-1", "post-1").block().getUser().getId());
        }

        @Test
        void user_hasCorrectName() {
            assertEquals("Alice Smith",
                    client.queries.fetchUserAndPost("user-1", "post-1").block().getUser().getName());
        }

        @Test
        void post_isNotNull() {
            assertNotNull(client.queries.fetchUserAndPost("user-1", "post-1").block().getPost());
        }

        @Test
        void post_hasCorrectId() {
            assertEquals("post-1",
                    client.queries.fetchUserAndPost("user-1", "post-1").block().getPost().getId());
        }

        @Test
        void post_hasCorrectTitle() {
            assertEquals("Hello World",
                    client.queries.fetchUserAndPost("user-1", "post-1").block().getPost().getTitle());
        }

        @Test
        void post_author_hasCorrectId() {
            assertEquals("user-1",
                    client.queries.fetchUserAndPost("user-1", "post-1").block().getPost().getAuthor().getId());
        }
    }
    ...
    class FetchUserSummary {
        ...
        @Test
        void response_isNotNull() {
            assertNotNull(client.queries.fetchUserSummary("user-1").block());
        }

        @Test
        void getUser_isNotNull() {
            assertNotNull(client.queries.fetchUserSummary("user-1").block().getGetUser());
        }

        @Test
        void getUser_isProjectedType() {
            assertInstanceOf(User_IdNameStatus.class,
                    client.queries.fetchUserSummary("user-1").block().getGetUser());
        }

        @Test
        void getUser_hasCorrectId() {
            assertEquals("user-1",
                    client.queries.fetchUserSummary("user-1").block().getGetUser().getId());
        }

        @Test
        void getUser_hasCorrectName() {
            assertEquals("Alice Smith",
                    client.queries.fetchUserSummary("user-1").block().getGetUser().getName());
        }

        @Test
        void getUser_status_deserializesToEnum() {
            assertEquals(UserStatus.ACTIVE,
                    client.queries.fetchUserSummary("user-1").block().getGetUser().getStatus());
        }
    }
    ...
    class RunSearch {
        ...
        void results_containUserResult() {
            List<SearchResult> results = client.queries.runSearch("alice").block().getSearch();
