...
class ErrorCaptureTest {
    ...
    class GetUserOrErrorsSuccess {
        ...
        void returnsFullResponse() {
            assertNotNull(client.queries.getUserOrErrors("user-1").block());
        }

        @Test
        void data_isNotNull() {
            assertNotNull(client.queries.getUserOrErrors("user-1").block().getData());
        }

        @Test
        void data_user_hasCorrectId() {
            assertEquals("user-1",
                    client.queries.getUserOrErrors("user-1").block().getData().getGetUserOrErrors().getId());
        }

        @Test
        void errors_isNullOrEmpty_onSuccess() {
            GetUserOrErrorsFullResponse res = client.queries.getUserOrErrors("user-1").block();
            assertTrue(res.getErrors() == null || res.getErrors().isEmpty());
        }

        @Test
        void doesNotThrow_onSuccess() {
            assertDoesNotThrow(() -> client.queries.getUserOrErrors("user-1").block());
        }
    }
    ...
    class GetUserOrErrorsWithErrors {
        ...
        void doesNotThrow_whenServerReturnsError() {
            assertDoesNotThrow(() -> client.queries.getUserOrErrors("unknown-id").block());
        }

        @Test
        void errors_isNonEmpty_forUnknownId() {
            GetUserOrErrorsFullResponse res = client.queries.getUserOrErrors("unknown-id").block();
    ...
    class FindUserOrErrorsNullReturn {
        ...
        void doesNotThrow_forUnknownId() {
            assertDoesNotThrow(() -> client.queries.findUserOrErrors("user-99").block());
        }

        @Test
        void data_fieldIsNull_forUnknownId() {
            FindUserOrErrorsFullResponse res = client.queries.findUserOrErrors("user-99").block();
    ...
    class FindUserOrErrorsWithErrors {
        ...
        void doesNotThrow_forErrorId() {
            assertDoesNotThrow(() -> client.queries.findUserOrErrors("error-id").block());
        }

        @Test
        void errors_isNonEmpty_forErrorId() {
            FindUserOrErrorsFullResponse res = client.queries.findUserOrErrors("error-id").block();
            assertNotNull(res.getErrors());
            assertFalse(res.getErrors().isEmpty());
        }
    }
    ...
    class FindUserOrErrorsSuccess {
        ...
        void data_isPresent_forKnownId() {
            FindUserOrErrorsFullResponse res = client.queries.findUserOrErrors("user-1").block();
    ...
    class NonCaptureErrorsThrows {
        ...
        void getUser_throws_forUnknownId() {
            assertThrows(RuntimeException.class, () -> client.queries.getUser("unknown-id").block());
        }
    }
    ...
    class CreateUserOrErrorsMutation {
        ...
        void success_dataIsPresent() {
            CreateUserOrErrorsFullResponse res = client.mutations.createUserOrErrors(kCreateInput).block();
            assertNotNull(res.getData());
        ...
        @Test
        void doesNotThrow_whenServerReturnsError() {
            assertDoesNotThrow(() -> client.mutations.createUserOrErrors(kErrorInput).block());
        }

        @Test
        void errors_arePopulated_whenServerReturnsError() {
            CreateUserOrErrorsFullResponse res = client.mutations.createUserOrErrors(kErrorInput).block();
            assertNotNull(res.getErrors());
            assertFalse(res.getErrors().isEmpty());
        }
    }
