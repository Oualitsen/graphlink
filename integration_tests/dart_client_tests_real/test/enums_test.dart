...
void main() {
  ...
  group('enum serialization in query variables', () {
    test('UserStatus.ACTIVE filters correctly — returns only active users', () async {
      final res = await client.queries.listUsersByStatus(status: UserStatus.ACTIVE);
      expect(res.listUsersByStatus, isNotEmpty);
      expect(res.listUsersByStatus.every((u) => u.status == UserStatus.ACTIVE), isTrue);
    });

    test('UserStatus.INACTIVE filters correctly — returns only inactive users', () async {
      final res = await client.queries.listUsersByStatus(status: UserStatus.INACTIVE);
      expect(res.listUsersByStatus, isNotEmpty);
      expect(res.listUsersByStatus.every((u) => u.status == UserStatus.INACTIVE), isTrue);
    });

    test('UserStatus.SUSPENDED filters correctly — returns empty list', () async {
      final res = await client.queries.listUsersByStatus(status: UserStatus.SUSPENDED);
