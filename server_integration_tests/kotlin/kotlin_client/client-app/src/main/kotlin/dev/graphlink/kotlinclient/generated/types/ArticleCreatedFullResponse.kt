...
data class ArticleCreatedFullResponse(
    override val errors: List<GraphLinkError>? = null,
    val data: ArticleCreatedResponse? = null,
   ...
   companion object {
      fun fromJson(map: Map<String, Any?>): ArticleCreatedFullResponse = ArticleCreatedFullResponse(
          errors = (map["errors"] as? List<*>)?.map { e0 -> GraphLinkError.fromJson(e0 as Map<String, Any?>) },
          data = (map["data"] as? Map<*, *>)?.let { ArticleCreatedResponse.fromJson(it as Map<String, Any?>) },
      )
   }
}
