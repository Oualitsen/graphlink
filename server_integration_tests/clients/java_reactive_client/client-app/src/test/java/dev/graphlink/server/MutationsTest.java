...
public class MutationsTest {
    ...
    class DeleteArticle {
        ...
        void returnsTrueWhenArticleExists() {
            var created = client.mutations.createArticle(
                    ...
                    new CreateArticleFieldArgs(2)).block();
            var res = client.mutations.deleteArticle(created.getCreateArticle().getId()).block();
            assertTrue(res.getDeleteArticle());
        }

        @Test
        void returnsFalseWhenArticleDoesNotExist() {
            var res = client.mutations.deleteArticle("missing-id").block();
    ...
    class AckPriority {
        ...
        void enumMutationArgumentDeserializesReturnsConstantAck() {
            var res = client.mutations.ackPriority(Priority.HIGH).block();
