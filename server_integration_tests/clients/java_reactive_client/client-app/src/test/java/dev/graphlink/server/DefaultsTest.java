...
public class DefaultsTest {
    ...
    void inputFieldDefaultsAppliedWhenOmitted() {
        var c = client.queries.resolveConfig(ConfigInput.builder().build()).block().getResolveConfig();
    ...
    void explicitValuesOverrideDefaults() {
        var c = client.queries.resolveConfig(ConfigInput.builder()
                .pageSize(5).sort("desc").priority(Priority.HIGH).note("x").build()).block().getResolveConfig();
    ...
    void nestedObjectArgDefaultMergesWithFieldDefaults() {
        var range = client.queries.resolveRange(null).block().getResolveRange();
    ...
    void argumentLevelScalarDefaults() {
        assertEquals("Hi world!", client.queries.greet(null, null).block().getGreet());
        assertEquals("Hi Bob!Hi Bob!", client.queries.greet("Bob", 2).block().getGreet());
    }

    @Test
    void enumArgumentPassedValueRoundTripsOmittedUsesDefaultHigh() {
        assertEquals(Priority.LOW, client.queries.echoPriority(Priority.LOW).block().getEchoPriority());
        assertEquals(Priority.HIGH, client.queries.echoPriority(null).block().getEchoPriority());
    }
}
