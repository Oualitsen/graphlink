...
public class NestingTest {
    ...
    void enumListDepths123() {
        assertEquals(List.of(Color.RED, Color.GREEN, Color.BLUE), client.queries.colors1().block().getColors1());
        assertEquals(
                List.of(List.of(Color.RED), List.of(Color.GREEN, Color.BLUE)),
                client.queries.colors2().block().getColors2());
        assertEquals(
                List.of(List.of(List.of(Color.RED))),
                client.queries.colors3().block().getColors3());
    }

    @Test
    void typeListDepths123() {
        var b1 = client.queries.boxes1().block().getBoxes1();
        assertEquals(2, b1.size());

        var b2 = client.queries.boxes2().block().getBoxes2();
        assertEquals(2, b2.size());

        var b3 = client.queries.boxes3().block().getBoxes3();
        assertEquals("b1", b3.get(0).get(0).get(0).getId());
    }

    @Test
    void interfaceListDepths123() {
        var s1 = client.queries.shapes1().block().getShapes1();
        var kinds = s1.stream().map(s -> s.getKind()).toList();
        assertTrue(kinds.containsAll(List.of("circle", "square")));

        var s3 = client.queries.shapes3().block().getShapes3();
        assertNotNull(s3.get(0).get(0).get(0).getKind());
    }

    @Test
    void unionListDepths123() {
        var m1 = client.queries.media1().block().getMedia1();
        ...
        var m3 = client.queries.media3().block().getMedia3();
