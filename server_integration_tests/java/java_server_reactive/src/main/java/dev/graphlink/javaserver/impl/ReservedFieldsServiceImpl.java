...
public class ReservedFieldsServiceImpl implements ReservedFieldsService {
    ...
    @Override
    public Mono<ReservedFields> reserved() {
        return Mono.just(base().build());
    }

    @Override
    public Mono<ReservedFields> switch_(String class_, Integer return_) {
        return Mono.just(base().class_(class_).return_(return_ != null ? return_ : 0).build());
    }

    @Override
    public Mono<ReservedFields> echoReserved(ReservedInput input) {
