...
| `defaultClearButton` | `boolean` | `false` | Show a clear (X) button on generated text fields. Override per field with `TextFieldOptions(clearButton: true/false)` in the form's `TextConfig`. Hidden while the field is empty or disabled, replaced by the spinner during async validation, and never shown on password fields. The tooltip comes from `FormStrings.clear`. |
