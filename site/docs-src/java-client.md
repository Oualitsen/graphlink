...
description: Use the generated GraphLink Java client for type-safe GraphQL queries. No generics, injectable HttpClient/OkHttpClient, builder pattern inputs, file uploads, and an optional reactive mode (Reactor/RxJava3/Mutiny) — new GraphLinkClient(url) is all you need.
---
...
!!! info "toJson/fromJson generation is mandatory (v5.0.0+)"
    Every generated type, input, enum, and interface always gets `toJson`/`fromJson` — there is no config flag to suppress it. This matters because [identifier normalization](configuration.md#identifier-normalization) can make a field's generated Java name diverge from its wire name (e.g. a keyword-colliding or non-canonically-cased GraphQL field): code that calls generated getters is always working with the normalized name, and only `toJson`/`fromJson` know how to bridge back to the wire format.
...
// Usage — no builder needed
AddVehicleInput input = new AddVehicleInput("Toyota", "Camry", 2023, FuelType.GASOLINE, null);
System.out.println(input.brand()); // component accessor, no get prefix
```

Records require Java 16 or later. Mapping methods generated by `@glMapsTo` use component accessors automatically when `typeAsRecord` is `true`.

## Reactive client (v5.0.0+)

By default the Java client is blocking — every query/mutation method returns its result directly and subscriptions notify via a listener callback (see above). Set `"asyncStyle"` in `clientConfig.java` to generate a reactive client instead:

=== "JSON"

    ```json title="glink.json — reactive client"
    {
      "clientConfig": {
        "java": {
          "asyncStyle": "reactor",
...
=== "YAML"
    ...
    ```yaml title="glink.yaml — reactive client"
    clientConfig:
      java:
        asyncStyle: reactor
        reactiveHttpClient: jdk
...
| `asyncStyle` | Query/mutation return type | Subscription return type | Library |
|---|---|---|---|
| `"blocking"` (default) | `T` | listener callback | — |
| `"reactor"` | `Mono<T>` | `Flux<T>` | Project Reactor |
| `"rxjava3"` | `Single<T>` | `Observable<T>` | RxJava 3 |
| `"mutiny"` | `Uni<T>` | `Multi<T>` | SmallRye Mutiny |

```java title="Reactive query — asyncStyle: reactor"
Mono<GetVehicleResponse> res = client.queries.getVehicle("42");
res.subscribe(r -> System.out.println(r.getGetVehicle().getBrand()));
```

`reactiveHttpClient` selects the default adapter's transport:

- `"jdk"` (default) — uses `HttpClient.sendAsync`. Works with every `asyncStyle`, no extra dependency beyond the reactive library itself.
- `"webclient"` — uses Spring `WebClient`. Only compatible with `asyncStyle: "reactor"`.

File upload support is available for the Java client regardless of `asyncStyle` — see below.
