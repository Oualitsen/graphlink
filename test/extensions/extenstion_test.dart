...
void main() {
  ...
  ''');
    ...
    expect(animal.values.map((e) => e.value.token),
        containsAll(['dog', 'cat', 'horse']));
  });
  ...
  ''');
    ...
    expect(userInput.fields.map((e) => e.name.token),
        containsAll(['password', 'username']));
  ...
  ''');
    ...
    expect(myUnion.typeNames.map((e) => e.token),
        containsAll(['type1', 'type2', 'type3']));
