...
void main() {
  group('GLParser — operation definitions', () {
    test('simple query', () {
      ...
      expect(parser.queries.containsKey(const GLOperationKey('GetUser', GLQueryType.query)), true);
    });

    test('query type is query', () {
      final parser = GLParser();
      parser.parse('query GetUser { user }', validate: false);
      expect(parser.queries[const GLOperationKey('GetUser', GLQueryType.query)]!.type, GLQueryType.query);
    });

    test('mutation type is mutation', () {
      final parser = GLParser();
      parser.parse('mutation CreateUser { createUser }', validate: false);
      expect(parser.queries[const GLOperationKey('CreateUser', GLQueryType.mutation)]!.type, GLQueryType.mutation);
    });

    test('subscription type is subscription', () {
      final parser = GLParser();
      parser.parse('subscription OnUser { userUpdated }', validate: false);
      expect(parser.queries[const GLOperationKey('OnUser', GLQueryType.subscription)]!.type, GLQueryType.subscription);
    });

    test('query with single element', () {
      final parser = GLParser();
      parser.parse('query GetUser { user }', validate: false);
      final q = parser.queries[const GLOperationKey('GetUser', GLQueryType.query)]!;
    ...
    test('query with multiple elements', () {
      ...
      expect(parser.queries[const GLOperationKey('GetData', GLQueryType.query)]!.elements.length, 3);
    });

    test('query element with alias', () {
      final parser = GLParser();
      parser.parse('query GetUser { me: user }', validate: false);
      final elem = parser.queries[const GLOperationKey('GetUser', GLQueryType.query)]!.elements.first;
    ...
    test('subscription enforces single root field', () {
      final parser = GLParser();
      parser.parse('subscription OnUser { userUpdated }', validate: false);
      expect(parser.queries[const GLOperationKey('OnUser', GLQueryType.subscription)]!.elements.length, 1);
    });
