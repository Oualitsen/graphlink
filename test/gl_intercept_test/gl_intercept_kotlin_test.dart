...
// Kotlin Spring controllers call `runBefore` as a plain leading statement
// (blocking mode) — mirrors the Express/TS and Java coverage in this folder:
// one root-handler resolver, one batch-mapping field, plus an un-intercepted
// sibling of each to prove the directive is opt-in, not blanket coverage.
const _schema = '''
type Query {
...
void main() {
  KotlinSpringServerSerializer serializer(GLParser g) =>
      KotlinSpringServerSerializer(g, packageName: 'dev.graphlink.test');
  ...
  test("intercepted root handler calls runBefore before the service call", () {
    ...
    final runBeforeIdx =
        out.indexOf('interceptor.runBefore(GlInterceptorTag.$tagMember, "getSecretArticle", listOf(id), graphQLContext)');
  ...
  test("intercepted batch mapping field calls runBefore before the service call", () {
    ...
    final runBeforeIdx =
        out.indexOf('interceptor.runBefore(GlInterceptorTag.$tagMember, "Publisher.books", listOf(), graphQLContext)');
