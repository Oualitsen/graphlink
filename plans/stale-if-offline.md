...
### 2. `gl_grammar_cache_extension.dart`
- Read and validate `staleIfOffline` from the directive

### 3. Code generation — `_GraphLinkPartialQuery`
- Add `staleIfOffline` field to the generated class

### 4. Code generation — query method
- Pass `staleIfOffline` value when constructing `_GraphLinkPartialQuery`

### 5. Code generation — `_getFromCache`
...
### 6. Code generation — query execution flow
